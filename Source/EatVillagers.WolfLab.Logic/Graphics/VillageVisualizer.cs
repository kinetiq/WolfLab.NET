...
namespace EatVillagers.WolfLab.Logic.Graphics
{
    public class VillageVisualizer
    {
        public void Show(List<PlayerModel> players)
        {
            ...
            var stats = playersByTeam.Where(p => p.IsAlive).Select(p => p.Name + ": " + p.NetAggro()).ToList();
            ...
            if (players.Any(p => !p.IsAlive))
            {
                ...
                var deadStats = playersByTeam.Where(p => !p.IsAlive).Select(p => p.Name + ": " + p.NetAggro()).ToList();
